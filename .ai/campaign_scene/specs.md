...
* Campos de ruleset como `system`, `sourcePack`, `bestiaryCreatureId`, `hitPoints`, `armorClass`, `spellbook`, `inventoryItemId`, condicoes ou efeitos ativos nao pertencem ao contrato base.
...
* Quando um token controlavel por Player passa a definir ou alterar a cena visivel do seu dono, o socket dono deve receber snapshot da cena visivel atualizada, mesmo que os demais sockets recebam apenas delta de token.
