...
test('removing the individual binding restores the campaign spell default', () => {
  const inherited = binding('default', defaultSource, {
    template: {
      shape: 'CIRCLE',
      originMode: 'FREE_POINT',
  ...
  assert.equal(resolved.presentation?.effective?.id, 'default')
  assert.deepEqual(resolved.presentation?.effective?.source, defaultSource)
  assert.equal(resolved.activation?.kind, 'AREA_PLACEMENT')
  if (resolved.activation?.kind !== 'AREA_PLACEMENT') return
  assert.equal(resolved.activation.template.dimensions.radius, 6)
})

test('cone and line activations from the source Token exclude the caster', () => {
  for (const shape of ['CONE', 'LINE'] as const) {
    const activation = areaEffectBindingToActivation(binding(shape, defaultSource, {
      template: {
        shape,
        originMode: 'SOURCE_TOKEN',
        placementMode: 'DIRECTIONAL',
        dimensions: shape === 'CONE'
          ? { length: 6, angleDegrees: 60 }
          : { length: 6, width: 1.5 },
        includesOrigin: true,
      },
    }))
    ...
    assert.equal(activation?.kind, 'AREA_PLACEMENT')
    if (activation?.kind !== 'AREA_PLACEMENT') continue
    assert.equal(activation.template.includesOrigin, false)
  }
})
