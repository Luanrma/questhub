...
test('PF2e icon assets resolve supported local image paths', () => {
  ...
  assert.equal(normalizedPath(svg.absolutePath).endsWith('default-icons/npc.svg'), true)

  assert.ok(webp)
  assert.equal(webp.contentType, 'image/webp')
  assert.equal(normalizedPath(webp.absolutePath).endsWith('spells/magic-missile.webp'), true)
})
