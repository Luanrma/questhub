...
test('validateTradeEligibility allows active members from the same campaign', () => {
  const result = validateTradeEligibility({
    authenticatedUserId: 'user-1',
    campaignId: 'campaign-1',
    from: activeLink({ actorId: 'actor-1', userId: 'user-1' }),
    to: activeLink({ actorId: 'actor-2', userId: 'user-2' }),
  })
...
test('validateTradeEligibility rejects trade when origin is not controlled by authenticated user', () => {
  const result = validateTradeEligibility({
    authenticatedUserId: 'user-1',
    campaignId: 'campaign-1',
    from: activeLink({ actorId: 'actor-1', userId: 'user-2' }),
    to: activeLink({ actorId: 'actor-2', userId: 'user-3' }),
  })

  assert.deepEqual(result, { ok: false, reason: 'FROM_CHARACTER_FORBIDDEN' })
})

test('validateTradeEligibility rejects members from different campaigns', () => {
  const result = validateTradeEligibility({
    authenticatedUserId: 'user-1',
    campaignId: 'campaign-1',
    from: activeLink({ actorId: 'actor-1', userId: 'user-1' }),
    to: activeLink({ campaignId: 'campaign-2', actorId: 'actor-2', userId: 'user-2' }),
  })

  assert.deepEqual(result, { ok: false, reason: 'DIFFERENT_CAMPAIGN' })
})

test('validateTradeEligibility rejects non-active origin or destination members', () => {
  assert.deepEqual(
    validateTradeEligibility({
      authenticatedUserId: 'user-1',
      campaignId: 'campaign-1',
      from: activeLink({ actorId: 'actor-1', userId: 'user-1', status: 'PENDING' }),
      to: activeLink({ actorId: 'actor-2', userId: 'user-2' }),
    }),
  ...
  assert.deepEqual(
    validateTradeEligibility({
      ...
      campaignId: 'campaign-1',
      from: activeLink({ actorId: 'actor-1', userId: 'user-1' }),
      to: activeLink({ actorId: 'actor-2', userId: 'user-2', status: 'DEAD' }),
    }),
