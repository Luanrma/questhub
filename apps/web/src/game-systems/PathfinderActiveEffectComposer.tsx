...
export function PathfinderActiveEffectComposer({ campaignId, actorId }: Props) {
  ...
  const modal = open && typeof document !== 'undefined'
    ? createPortal(
      <div
        className="fixed inset-0 z-[360] flex items-center justify-center bg-black/55 p-3 backdrop-blur-sm sm:p-4"
        role="presentation"
        onPointerDown={(event) => {
      ...
      >
        <section
          role="dialog"
          aria-modal="true"
          aria-label="Aplicar efeito Pathfinder 2e"
          className="flex h-[min(46rem,calc(100dvh-1.5rem))] max-h-[calc(100dvh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-xl border border-[#8c7a60]/60 bg-[#e1d3bd] text-[#352d24] shadow-2xl sm:max-h-[calc(100dvh-2rem)]"
          onPointerDown={(event) => event.stopPropagation()}
        >
          <header className="flex shrink-0 items-center justify-between gap-3 border-b border-[#7c6d59]/20 px-4 py-3">
            <div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 pb-3" data-pf2e-effect-definition-list>
          ...
          <footer className="shrink-0 border-t border-[#7c6d59]/20 bg-[#d7c8af] p-3" data-pf2e-effect-apply-footer>
            {selected?.valued ? (
            ...
            <div className="flex items-center justify-between gap-3">
              <span className="min-w-0 flex-1 truncate text-xs text-[#6b5d4d]">
