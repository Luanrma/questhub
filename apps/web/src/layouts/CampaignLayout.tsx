...
type CampaignPanelId = 'sessions' | 'members' | 'players' | 'journal' | 'settings'
...
export function CampaignLayout() {
  ...
  function renderCampaignPanel(panelId: CampaignPanelId) {
    ...
    if (panelId === 'members') return <PlaceholderPage title="Personagens" />
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        const ch = await api<MyCampaignCharacter>(`/api/campaigns/${campaignId}/my-actor${selectedCharacterQuery}`)
        setMyActor(ch)
