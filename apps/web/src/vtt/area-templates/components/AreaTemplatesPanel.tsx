...
import { areaTemplateForMeterEditor } from '../domain/measurement'
...
const shapeLabels: Record<AreaShape, string> = {
  CIRCLE: 'Circulo', CONE: 'Cone', LINE: 'Linha', RECTANGLE: 'Retangulo', RING: 'Anel', POLYGON: 'Poligono',
}

function dimensionsForShape(shape: AreaShape, current: AreaTemplateInput['dimensions']) {
  if (shape === 'CIRCLE') return { radius: current.radius ?? 3 }
  if (shape === 'CONE') return { length: current.length ?? 6, angleDegrees: current.angleDegrees ?? 60, startWidth: current.startWidth ?? 0 }
  if (shape === 'LINE' || shape === 'RECTANGLE') return { length: current.length ?? 6, width: current.width ?? 1 }
...
function TemplateEditor({ template, gridScale, onCancel, onSave }: {
  ...
  const [form, setForm] = useState<AreaTemplateInput>(() => areaTemplateForMeterEditor(template ?? defaultAreaTemplateInput, gridScale.metersPerCell))
  const [saving, setSaving] = useState(false)
  const setDimension = (key: string, value: number) => setForm((current) => ({ ...current, dimensions: { ...current.dimensions, [key]: value } }))
  const directional = form.shape === 'CONE' || form.shape === 'LINE' || form.shape === 'RECTANGLE'
  const dimensionLabel = (label: string) => `${label} (m)`
  ...
  return (
    <form className="grid min-w-0 gap-3 overflow-x-hidden [&>*]:min-w-0" onSubmit={submit}>
      <div className="grid min-w-0 grid-cols-2 gap-2 [&>*]:min-w-0">
        <label className="col-span-2 text-xs text-zinc-400">Nome<input required maxLength={areaTemplateNameMaxLength} value={form.name} onChange={(event) => setForm({ ...form, name: event.currentTarget.value })} className="mt-1 w-full rounded-md border border-white/10 bg-black/40 px-3 py-2 text-sm text-white" /></label>
        <label className="text-xs text-zinc-400">Forma<select value={form.shape} onChange={(event) => { const shape = event.currentTarget.value as AreaShape; setForm({ ...form, shape, dimensions: dimensionsForShape(shape, form.dimensions), placementMode: shape === 'CIRCLE' || shape === 'RING' ? 'POINT' : 'DIRECTIONAL' }) }} className="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-2 py-2 text-sm">{(['CIRCLE', 'CONE', 'LINE', 'RECTANGLE'] as AreaShape[]).map((shape) => <option key={shape} value={shape}>{shapeLabels[shape]}</option>)}</select></label>
        <label className="text-xs text-zinc-400">Unidade das dimensoes<div className="mt-1 rounded-md border border-white/10 bg-zinc-950 px-2 py-2 text-sm text-zinc-200">Metros (m)</div></label>
        <p className="col-span-2 rounded-md border border-sky-300/15 bg-sky-500/5 px-2.5 py-2 text-[10px] leading-4 text-sky-100/75">Nesta cena, cada celula representa {gridScale.metersPerCell}m. Exemplo: um raio de 3m ocupa {Number((3 / gridScale.metersPerCell).toFixed(2))} celulas.</p>
      </div>

      <div className="grid min-w-0 grid-cols-2 gap-2 rounded-md border border-white/10 p-2 [&>*]:min-w-0">
        {(form.shape === 'CIRCLE' || form.shape === 'RING') ? <label className="text-xs text-zinc-400">{dimensionLabel('Raio externo')}<input type="number" min="0.1" step="0.1" value={form.dimensions.radius ?? 1} onChange={(event) => setDimension('radius', Number(event.currentTarget.value))} className="mt-1 w-full rounded bg-black/40 px-2 py-1.5 text-white" /></label> : null}
        {form.shape === 'RING' ? <label className="text-xs text-zinc-400">{dimensionLabel('Raio interno')}<input type="number" min="0" step="0.1" value={form.dimensions.innerRadius ?? 0} onChange={(event) => setDimension('innerRadius', Number(event.currentTarget.value))} className="mt-1 w-full rounded bg-black/40 px-2 py-1.5 text-white" /></label> : null}
        {directional ? <label className="text-xs text-zinc-400">{dimensionLabel('Comprimento')}<input type="number" min="0.1" step="0.1" value={form.dimensions.length ?? 1} onChange={(event) => setDimension('length', Number(event.currentTarget.value))} className="mt-1 w-full rounded bg-black/40 px-2 py-1.5 text-white" /></label> : null}
        {(form.shape === 'LINE' || form.shape === 'RECTANGLE') ? <label className="text-xs text-zinc-400">{dimensionLabel('Largura')}<input type="number" min="0.1" step="0.1" value={form.dimensions.width ?? 1} onChange={(event) => setDimension('width', Number(event.currentTarget.value))} className="mt-1 w-full rounded bg-black/40 px-2 py-1.5 text-white" /></label> : null}
        {form.shape === 'CONE' ? <label className="text-xs text-zinc-400">Abertura (graus)<input type="number" min="1" max="359" value={form.dimensions.angleDegrees ?? 60} onChange={(event) => setDimension('angleDegrees', Number(event.currentTarget.value))} className="mt-1 w-full rounded bg-black/40 px-2 py-1.5 text-white" /></label> : null}
      </div>

      <div className="grid min-w-0 grid-cols-2 gap-2 [&>*]:min-w-0">
        <label className="text-xs text-zinc-400">Persistencia<select value={form.persistenceMode} onChange={(event) => setForm({ ...form, persistenceMode: event.currentTarget.value as AreaTemplateInput['persistenceMode'] })} className="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-2 py-2 text-sm"><option value="PREVIEW_ONLY">Somente previa</option><option value="INSTANT">Instantanea</option><option value="PERSISTENT">Persistente</option></select></label>
        ...
        <label className="text-xs text-zinc-400">Visibilidade<select value={form.visibility} onChange={(event) => setForm({ ...form, visibility: event.currentTarget.value as AreaTemplateInput['visibility'] })} className="mt-1 w-full min-w-0 rounded-md border border-white/10 bg-zinc-950 px-2 py-2 text-sm"><option value="MASTER_ONLY">Somente Mestre</option><option value="ALL_PLAYERS">Todos</option></select></label>
        <label className="col-span-2 flex min-w-0 items-start gap-2 rounded-md border border-white/10 bg-white/[0.03] px-2.5 py-2 text-xs text-zinc-300"><input type="checkbox" checked={form.tokenIntersectionRule === 'COVERED_CELLS'} onChange={(event) => setForm({ ...form, tokenIntersectionRule: event.currentTarget.checked ? 'COVERED_CELLS' : 'ANY_OVERLAP' })} className="mt-0.5 h-3.5 w-3.5 shrink-0 accent-orange-500" /><span className="min-w-0"><span className="block font-semibold text-zinc-200">Usar celulas do grid para atingir tokens</span><span className="mt-0.5 block text-[10px] leading-4 text-zinc-500">Marcado: o token e atingido ao ocupar uma celula afetada. Desmarcado: somente a forma geometrica exata importa.</span></span></label>
        {form.tokenIntersectionRule === 'COVERED_CELLS' ? <fieldset className="col-span-2 grid min-w-0 gap-1.5 rounded-md border border-orange-300/15 bg-orange-500/[0.04] p-2.5"><legend className="px-1 text-[10px] font-semibold uppercase text-orange-100/80">Quando uma celula conta como afetada?</legend>{([
      ...
      <label className="text-xs text-zinc-400">Efeito visual<select value={form.style.visualEffect ?? 'DEFAULT'} onChange={(event) => setForm({ ...form, style: { ...form.style, visualEffect: event.currentTarget.value as AreaTemplateInput['style']['visualEffect'] } })} className="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-2 py-2 text-sm"><option value="DEFAULT">Padrao</option><option value="FIRE">Fogo</option><option value="ELECTRIC">Eletrico</option><option value="HEALING">Cura</option><option value="EARTH">Terra (terremoto)</option><option value="VINES">Videiras</option><option value="LEAVES">Plantas (rajada de folhas)</option></select></label>
      <label className="text-xs text-zinc-400">Descricao<textarea maxLength={1000} value={form.description ?? ''} onChange={(event) => setForm({ ...form, description: event.currentTarget.value || undefined })} className="mt-1 min-h-16 w-full rounded-md border border-white/10 bg-black/40 px-3 py-2 text-sm text-white" /></label>
