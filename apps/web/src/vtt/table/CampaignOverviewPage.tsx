...
export function CampaignOverviewPage({
  ...
  const fogVisionToken = isMaster
    ...
    : controlledVisionTokens.find((token) => token.actorId === myActor?.id) ?? controlledVisionTokens[0] ?? null
  ...
  useEffect(() => {
    ...
    function onTokenRemoved(payload: VttTokenRemovedPayload) {
      ...
      const isOwnRemovedToken = !isMaster && payload.actorId === myActor?.id
  ...
  }, [socket, campaignId, gridSettings.shape, isMaster, activeScene?.id, me?.id, myActor?.id, startSmoothTokenMovement, resetLocalFogExploration, flushFogExploration])
  ...
  function movePlayerToken(token: VttPlayerToken, position: VttPlayerToken['position']) {
    ...
    const isOwnerMove = sessionActive && token.controllerUserId === me?.id && myActor?.role === 'PLAYER'
  ...
  function beginMeasuredMovement(event: React.PointerEvent<HTMLButtonElement>, token: VttPlayerToken) {
    ...
    const canMoveToken = Boolean(isMaster) || (sessionActive && token.controllerUserId === me?.id && myActor?.role === 'PLAYER')
              ...
              <PlayerToken
                ...
                canDrag={
                  !movingTokenIds.has(token.id) && (
                    (sessionActive && !activeCombat && token.controllerUserId === me?.id && myActor?.role === 'PLAYER') ||
                    Boolean(isMaster)
                  )
                }
                canSelect={
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-[11px] uppercase text-zinc-500">
                          {token.category === 'MAIN' ? 'Main Actor' : token.category === 'SECONDARY' ? 'Secondary' : 'Somente Mestre'}
                        </span>
                      </span>
      ...
      >
        ...
        <div className={rightPanelCollapsed ? 'hidden h-full min-h-0 flex-col gap-3' : 'relative flex h-full min-h-0 flex-col gap-3'}>
          ...
          <div className="min-h-0 flex-1 overflow-hidden">
            ...
            {rightPanelTab === 'chat' && campaignId ? <CampaignChat campaignId={campaignId} actorId={campaign?.myActorId} enabled={Boolean(campaign?.isOnline && campaign?.myStatus === 'ACTIVE')} className="h-full min-h-0" /> : null}
          </div>
        </div>
      </aside>
