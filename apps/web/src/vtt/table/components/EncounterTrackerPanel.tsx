...
import type { VttEncounterParticipant, VttEncounterState, VttPlayerToken } from '../domain/types'
...
export function EncounterTrackerPanel({
  ...
  const subtitle = encounter ? `Rodada ${encounter.round}${activeParticipant ? ' - turno atual' : ''}` : `${selectedTokens.length}/${tokenCount} token${tokenCount === 1 ? '' : 's'} selecionados`
