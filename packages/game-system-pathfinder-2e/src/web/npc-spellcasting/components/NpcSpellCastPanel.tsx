...
import { fetchNpcCastOptions, castNpcSpell, restNpcSpellbook, refocusNpcSpellbook } from '../api'
import { clampFloatingPanelPosition } from '../../shared/floatingPanel'
import type { FloatingPanelPosition } from '../../shared/floatingPanel'
import type { NpcCastOptionsResponse, NpcCastRequestPayload } from '../types'
import type { Pathfinder2eCastSourcePayload, Pathfinder2eCastableSpellView } from '../../spell-casting/types'
...
export type Pathfinder2eNpcTargetSelectionRequest = {
  spellName: string
  ...
  cast: { entryId: string; spellId: string; source: Pathfinder2eCastSourcePayload } & Pick<NpcCastRequestPayload, 'clientCastId'>
}
...
type Props = {
  ...
  casterName: string
  onClose: () => void
  onRequestTargetSelection: (request: Pathfinder2eNpcTargetSelectionRequest) => void
  onCastCommitted: (result: { spellName: string; consumed: string }) => void
}

export function NpcSpellCastPanel({ campaignId, definitionId, casterName, onClose, onRequestTargetSelection, onCastCommitted }: Props) {
  const [options, setOptions] = useState<NpcCastOptionsResponse | null>(null)
  ...
  const reload = useCallback(() => {
    setLoading(true)
    setError(null)
    fetchNpcCastOptions(campaignId, definitionId)
  ...
  async function performCast(entryId: string, spell: Pathfinder2eCastableSpellView, source: Pathfinder2eCastSourcePayload) {
    ...
    if (spell.area) {
      setError('Magias com area ainda nao sao suportadas para conjuracao de NPC.')
      return
    }

    if (spell.targetProfile.kind === 'count') {
      onRequestTargetSelection({
        spellName: spell.name,
        targetProfile: spell.targetProfile,
        resolution: spell.resolution,
        cast: { clientCastId, entryId, spellId: spell.spellId, source },
      })
      return
    }
    ...
    setError(null)
    try {
      const result = await castNpcSpell(campaignId, definitionId, { clientCastId, entryId, spellId: spell.spellId, source })
  ...
  async function performRecovery(action: 'rest' | 'refocus') {
    ...
    try {
      if (action === 'rest') {
        await restNpcSpellbook(campaignId, definitionId)
        setFeedback('Descanso diario: recursos restaurados')
      } else {
        await refocusNpcSpellbook(campaignId, definitionId)
      ...
      return (
        <>
          {castableRanks.map((slot) => (
            <CastButton key={slot.rank} disabled={busy} onClick={() => performCast(entryId, spell, { kind: 'spontaneous', rank: slot.rank })}>
    ...
    >
      ...
      >
        <div className="flex min-w-0 items-center gap-2 text-sm font-semibold">
          ...
          <Wand2 className="h-4 w-4 shrink-0 text-purple-300" />
          <span className="truncate">Magias de NPC — {casterName}</span>
        </div>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
        ...
        {!loading && options && options.entries.length === 0 ? (
          <p className="py-4 text-center text-sm text-zinc-400">Nenhuma entrada de conjuracao configurada para este NPC.</p>
        ) : null}
        ...
        {options?.entries.map((entry) => (
          <div key={entry.entryId} className="mt-2">
            ...
            {entry.spells.length === 0 ? <p className="px-1 py-1 text-xs text-zinc-500">Sem magias nesta entrada.</p> : null}
            {entry.spells.map((spell) => (
              <div key={spell.spellId} className="mt-1 rounded-md border border-white/5 bg-white/[0.03] px-2 py-1.5">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span className="shrink-0 text-[10px] text-zinc-500">{spell.isCantrip ? 'Truque' : `Rank ${spell.rank}`}</span>
                </div>
                <div className="mt-0.5 text-[10px] text-zinc-500">
                  {spell.area ? `Area: ${spell.area.type} ${spell.area.valueFeet} ft (nao suportado para NPC)` : 'Sem area'}
...
function CastButton({ disabled, onClick, children }: { disabled: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      disabled={disabled}
      className="rounded-md bg-purple-500/20 px-2 py-1 text-xs font-medium text-purple-200 transition hover:bg-purple-500/35 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
      onClick={onClick}
    >
      {children}
    </button>
